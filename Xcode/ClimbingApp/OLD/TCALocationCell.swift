...
class TCALocationCell: UICollectionViewCell, ReuseIdentifiable {
    ...
    public var numberOfClimbs: Int = 0 {
        didSet {
            climbsLabel.title = "\(numberOfClimbs.description) Climbs"
        }
    }
    ...
    private var climbsLabel = TCAButton()
    ...
    override init(frame: CGRect) {
        ...
        // Setup climbs label.
        climbsLabel.image = UIImage(systemName: Constants.climbsLabelIconName, withConfiguration: UIImage.SymbolConfiguration(scale: .small))
        climbsLabel.font = UIFont.roundedFont(forTextStyle: .caption1).bold()
        climbsLabel.imageColor = .white
        climbsLabel.titleColor = .white
